...
def synthesize_speech(text, language_code="en-US", voice_gender="NEUTRAL"):
    """Synthesizes speech from the input string of text, ignoring emojis (only alphabets, numbers, and basic punctuation are considered)."""
    if not tts_client:
        print("⚠️ TTS client not available. Cannot synthesize speech.")
        return None

    # Remove emojis and keep only alphabets, numbers, and basic punctuation
    import re
    ...
    try:
        print(f"Synthesizing speech for text (Lang: {language_code}): {filtered_text[:50]}...")
